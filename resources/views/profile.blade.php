...
<header class="flex items-center justify-between p-6">
    ...
    <div class="flex gap-2">
        ...
        <a href="{{ route('verification.notice') }}" class="rounded-md bg-gray-200 py-2 px-4 font-semibold text-gray-900 shadow-lg transition duration-150 ease-in-out hover:bg-gray-300 hover:shadow-xl focus:shadow-xl focus:outline-none focus:ring-2 focus:ring-gray-300 focus:ring-offset-2">Verification</a>
...
<main>
    <div class="m-6 mb-12 rounded-xl p-6 shadow-xl sm:p-10">
        ...
        <a href="{{ route('news.create') }} " class="rounded-md bg-gray-200 py-2 px-4 font-semibold text-gray-900 shadow-lg transition duration-150 ease-in-out hover:bg-gray-300 hover:shadow-xl focus:shadow-xl focus:outline-none focus:ring-2 focus:ring-gray-300 focus:ring-offset-2">Создать новость</a>

        <p class="card-text">Новости:
        </p>
            @foreach($news as $new)
            <div>
                @if(auth()->user()->id === $new->user_id)
                    ...
                    <form action="{{ route('news.destroy', $new->id) }}" method="post">
                        @csrf
                        @method('delete')
                        <input type="submit" value="Delete" class="rounded-md bg-gray-200 py-2 px-4 font-semibold text-gray-900 shadow-lg transition duration-150 ease-in-out hover:bg-gray-300 hover:shadow-xl focus:shadow-xl focus:outline-none focus:ring-2 focus:ring-gray-300 focus:ring-offset-2">
                    </form>
                    <a href="{{ route('news.edit', $new->id) }}" class="rounded-md bg-gray-200 py-2 px-4 font-semibold text-gray-900 shadow-lg transition duration-150 ease-in-out hover:bg-gray-300 hover:shadow-xl focus:shadow-xl focus:outline-none focus:ring-2 focus:ring-gray-300 focus:ring-offset-2">Изменить</a>
                @endif
            </div>
            @endforeach
